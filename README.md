...
- **每日自动更新**：GitHub Actions 每天 15:15（北京时间）自动抓数并部署，手机外网随时可看，无需电脑开机。公司基本面（控股股东 / 分红 / 财务绩效 / 高管增减持 / 简介）已纳入同一 daily-update 流程，带 30 天缓存、每日只刷新过期项，不会全量重抓 992 只股票。
...
> 注：K线（大文件）由云端任务每天现场生成，不提交进仓库；**公司基本面** (`data/company/`) 会提交进仓库并带 30 天缓存，每天仅增量刷新过期项，因此仓库体积可控。仓库里长期保留：股票池、指标历史、公司基本面。
