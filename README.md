...
- **首页排名区块**：列表上方三个榜单 ——（1）**银行股息率 Top10**（由大到小）；（2）**连续分红≥5年 · 近1月涨幅 Top10**；（3）**连续分红≥5年 · 近1月跌幅 Top10**；点击榜单个股直接进入详情。
- **标签筛选页**：标签栏**移除“基金持仓”**，**新增“国有企业”**；点击任意标签（含“国有企业”）打开**站内标签页**，列出所有带该标签的股票。
- **浏览器返回**：进入个股详情 / 标签页后，点页面“← 返回列表”按钮或浏览器**返回键**，均回到上一页（基于 URL hash 路由）。
- **红利/宽基基金页**：顶部「红利/宽基基金」标签打开独立列表页（`#/funds`），自动筛选**红利类基金 + 宽基指数基金**（沪深300 / 中证500 / 中证1000 / 上证50 / 创业板指 / 科创50 / 深证100 / 上证180 / 中证800 等，**剔除行业/主题/海外/债基/货基**）；每行展示 **近1/3/6/12月涨跌幅、近12月分红次数、分红比率**（近12月每份分红 ÷ 当前净值）、**市盈率、市净率、当前规模、过去1/3/6/12月规模**（过去规模后括号显示「当前−过去」差值与增减%，**红涨绿跌**），可搜索、可点表头排序；点任意一行进入 `#/fund/CODE`，用 ECharts 画**该基金自身的净值走势**。数据源：天天基金 pingzhongdata（净值/规模/分红）+ 乐咕乐股 index-basic（指数 PE/PB/股息率），均免登录。
- **每日自动更新**：GitHub Actions 每天 15:15（北京时间）自动抓数并部署，手机外网随时可看，无需电脑开机。公司基本面（控股股东 / 分红 / 财务绩效 / 高管增减持 / 简介）已纳入同一 daily-update 流程，带 30 天缓存、每日只刷新过期项，不会全量重抓 992 只股票。基金数据由同一流程内 `python pipeline/funds.py` 现场生成并部署。
